...
namespace glfw
{
  ...
  void Viewer::init_snake(int num_of_arms)
  {
	  ...
	  int i; 
	   ...
	   irrklang::ISound* snd =engine->play2D("audio/breakout.mp3", true,false,true);
	   snd->setVolume(0.1f);
	   snd->drop();
  ...
  void Viewer::reset_level()
  {
	  ...
	  for (i = 1; i <= snake_arms; i++)
	  {
		  ...
		  data_list[i + snake_arms].random_palce();
		  ...
		  for (int j = i + 1; j <= snake_arms; j++)
		  {
			 if (!data_list[j + snake_arms].is_visible)
			  ...
			  if (dis < base_dis)
			  {
				  ...
				  irrklang::ISound* snd = engine->play3D("audio/bounce.wav", center, false, true, true);
	  ...
	  for (int i = 1; i <= snake_arms; i++)
	  {
		//  if (!data_list[i + snake_arms].is_visible)
			  ...
			  head, MakeTrans() * data_list[i + snake_arms].MakeTrans_noscale()))
		  {
			  ...
			  irrklang::ISound* snd = engine->play3D("audio/smb3_coin.wav", center, false, true, true);
			  snd->setMinDistance(10.0f);
			  snd->setVolume(0.8f);
			  snd->setIsPaused(false);
			  snd->drop();
