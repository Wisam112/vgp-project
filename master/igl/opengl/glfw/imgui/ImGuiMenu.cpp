...
IGL_INLINE void ImGuiMenu::init(Renderer *_viewer)
{
...
IGL_INLINE void ImGuiMenu::draw_viewer_menu()
{
  ...
  if (ImGui::CollapsingHeader("Workspace", ImGuiTreeNodeFlags_DefaultOpen))
  {
    ...
    {
      viewer->GetScene()->load_scene();
    }
    ImGui::SameLine(0, p);
    if (ImGui::Button("Save##Workspace", ImVec2((w-p)/2.f, 0)))
    {
      viewer->GetScene()->save_scene();
    }
  }
  ...
  if (ImGui::CollapsingHeader("Mesh", ImGuiTreeNodeFlags_DefaultOpen))
  {
    ...
    {
      viewer->GetScene()->open_dialog_load_mesh();
    }
    ImGui::SameLine(0, p);
    if (ImGui::Button("Save##Mesh", ImVec2((w-p)/2.f, 0)))
    {
      viewer->GetScene()->open_dialog_save_mesh();
    }
  }
  ...
  if (ImGui::CollapsingHeader("Viewing Options", ImGuiTreeNodeFlags_DefaultOpen))
  {
    ...
    {
      viewer->core().align_camera_center(viewer->GetScene()->data().V, viewer->GetScene()->data().F);
    }
    if (ImGui::Button("Snap canonical view", ImVec2(-1, 0)))
    {
     // viewer->GetScene()->snap_to_canonical_quaternion();
    }
  ...
  {
    if (ImGui::Checkbox("Face-based", &(viewer->GetScene()->data().face_based)))
    {
      viewer->GetScene()->data().dirty = MeshGL::DIRTY_ALL;
    }
    make_checkbox("Show texture", viewer->GetScene()->data().show_texture);
    if (ImGui::Checkbox("Invert normals", &(viewer->GetScene()->data().invert_normals)))
    {
      viewer->GetScene()->data().dirty |= igl::opengl::MeshGL::DIRTY_NORMAL;
    }
    make_checkbox("Show overlay", viewer->GetScene()->data().show_overlay);
    make_checkbox("Show overlay depth", viewer->GetScene()->data().show_overlay_depth);
    ImGui::ColorEdit4("Background", viewer->core().background_color.data(),
        ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_PickerHueWheel);
    ImGui::ColorEdit4("Line color", viewer->GetScene()->data().line_color.data(),
        ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_PickerHueWheel);
    ImGui::PushItemWidth(ImGui::GetWindowWidth() * 0.3f);
    ImGui::DragFloat("Shininess", &(viewer->GetScene()->data().shininess), 0.05f, 0.0f, 100.0f);
    ImGui::PopItemWidth();
  }
  ...
  {
    make_checkbox("Wireframe", viewer->GetScene()->data().show_lines);
    make_checkbox("Fill", viewer->GetScene()->data().show_faces);
    ImGui::Checkbox("Show vertex labels", &(viewer->GetScene()->data().show_vertid));
    ImGui::Checkbox("Show faces labels", &(viewer->GetScene()->data().show_faceid));
  }
}
...
IGL_INLINE void ImGuiMenu::draw_labels_window()
{
  ...
  for (const auto & data : viewer->GetScene()->data_list)
  {
    draw_labels(data);
  }
  ImGui::End();
